...
Функция ФоновоеСозданиеАктов(Период) Экспорт
	...
	Запрос.Текст =
		...
		|	И НАЧАЛОПЕРИОДА(ДоговорыКонтрагентов.ДатаНачала, МЕСЯЦ) <= &Период
		|	И КОНЕЦПЕРИОДА(ДоговорыКонтрагентов.ДатаОкончания, МЕСЯЦ) >= &Период";
...
Функция СоздатьДокументРеализацияТоваровУслуг(Выборка, Период)
	...
	Если Выборка.РеализацияСсылка = Ссылка Тогда
		НовыйДокумент = Документы.РеализацияТоваровУслуг.СоздатьДокумент();
	Иначе
		НовыйДокумент = Выборка.РеализацияСсылка.ПолучитьОбъект();
	КонецЕсли;
